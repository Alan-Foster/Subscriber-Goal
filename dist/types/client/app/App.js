...
export const App = () => {
    const { state, loading, submitting, subscribe, simulateSubscribe, simulateIncrement, sendDebugRealtime, setError, notice, showNotice, } = useSubGoal();
    const [page, setPage] = useState('subGoal');
    const [showConfetti, setShowConfetti] = useState(false);
    const [confettiKey, setConfettiKey] = useState(0);
    const [confettiPieces, setConfettiPieces] = useState(70);
    const confettiTimeoutRef = useRef(null);
    const completedConfettiShownRef = useRef(false);
    const returnNoticeTimeoutRef = useRef(null);
    ...
    const triggerConfetti = useCallback(({ pieceCount = 70, durationMs = 2600, allowRestart = true, } = {}) => {
    ...
    if (state) {
        if (page === 'thanks') {
            content = (_jsx(ThanksPage, { state: state, onReturn: handleReturnToSubGoal, onVisitPromoSub: handleVisitPromo, onCelebrate: handleCelebrate }));
        }
        else if (page === 'completed') {
            content = (_jsx(CompletedPage, { state: state, onVisitPromoSub: handleVisitPromo, onCelebrate: handleCelebrate }));
        }
        else {
            content = (_jsx(SubGoalPage, { state: state, onSubscribe: handleSubscribe, onVisitPromoSub: handleVisitPromo, onCelebrate: handleCelebrate, isSubmitting: submitting, shareUsername: shareUsername, onShareUsernameChange: setShareUsername, notice: notice }));
        }
    }
    if (loading) {
        return _jsx(SkeletonPage, {});
    }
    return (_jsxs("div", { className: "relative flex h-[320px] w-full flex-col items-center justify-center overflow-hidden bg-[color:var(--sg-bg)] text-[color:var(--sg-text-primary)]", children: [showDebugControls ? (_jsxs("div", { className: "absolute left-1/2 top-4 z-20 flex -translate-x-1/2 items-center gap-1", children: [_jsx("button", { className: "cursor-pointer rounded-full border border-[color:var(--sg-border)] bg-[color:var(--sg-surface)] px-2 py-0.5 text-[10px] font-semibold text-[color:var(--sg-text-secondary)] shadow-sm transition hover:bg-[color:var(--sg-surface-muted)] disabled:cursor-not-allowed disabled:opacity-50", onClick: handleDebugIncrement, disabled: !state, children: "+1" }), _jsx("button", { className: "cursor-pointer rounded-full border border-[color:var(--sg-border)] bg-[color:var(--sg-surface)] px-2 py-0.5 text-[10px] font-semibold text-[color:var(--sg-text-secondary)] shadow-sm transition hover:bg-[color:var(--sg-surface-muted)] disabled:cursor-not-allowed disabled:opacity-50", onClick: handleDebugSubscribe, disabled: !state, children: "+Subscribe" })] })) : null, content ?? (_jsx("div", { className: "text-center text-sm text-[color:var(--sg-text-muted)]", children: "Unable to load Subscriber Goal data." })), showConfetti ? (_jsx(ConfettiBurst, { pieceCount: confettiPieces }, confettiKey)) : null] }));
};
