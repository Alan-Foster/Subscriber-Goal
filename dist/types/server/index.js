...
import { checkCompletionStatus, eraseFromRecentSubscribers, getSubGoalData, registerNewSubGoalPost, } from './data/subGoalData';
...
import { onModAction } from './triggers/modAction';
...
const getSettingsClient = () => context.settings;
const buildState = async (postId, options) => {
    ...
    return {
        ...
        subreddit: {
            ...
            name: subreddit.name,
            icon: subredditIcon,
            subscribers: options?.subscribersOverride ?? subreddit.numberOfSubscribers,
        },
    };
};
...
router.get('/api/refresh', async (_req, res) => {
    const { postId } = context;
    if (!postId) {
        res.status(400).json({
            status: 'error',
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? `Refresh failed: ${error.message}` : 'Unknown error during refresh';
        res.status(400).json({ status: 'error', message: errorMessage });
    }
});
router.post('/api/subscribe', async (_req, res) => {
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? `Subscription failed: ${error.message}` : 'Subscription failed.';
        res.status(400).json({ status: 'error', message: errorMessage });
    }
});
router.post('/internal/triggers/on-app-install', async (_req, res) => {
    try {
        await onAppChanged();
        res.json({ status: 'ok' });
    }
...
router.post('/internal/triggers/on-mod-action', async (req, res) => {
    try {
        const modAction = (req.body?.modAction ?? req.body);
...
router.post('/internal/scheduler/posts-updater-job', async (_req, res) => {
    try {
...
router.post('/api/debug/realtime', async (req, res) => {
    const body = req.body;
    if (!body || typeof body.nextCount !== 'number' || Number.isNaN(body.nextCount)) {
        res.status(400).json({ status: 'error', message: 'nextCount is required' });
        return;
    }
    ...
    try {
        ...
        res.status(400).json({ status: 'error', message: 'Failed to send debug realtime' });
    }
});
router.post('/internal/menu/create-goal', async (_req, res) => {
    try {
        const subreddit = await reddit.getCurrentSubreddit();
        const appSettings = await getAppSettings(getSettingsClient());
        const defaultGoal = getDefaultSubscriberGoal(subreddit.numberOfSubscribers);
        const shouldCrosspost = subreddit.name.toLowerCase() !== appSettings.promoSubreddit.toLowerCase();
        res.json({
            showForm: {
                name: 'createGoalForm',
                form: {
...
router.post('/internal/form/create-goal', async (req, res) => {
    ...
    try {
        ...
        await registerNewSubGoalPost(reddit, redis, appSettings, post, subscriberGoal, crosspost);
        await post.approve();
        await post.sticky();
        res.json({
...
router.post('/internal/menu/delete-goal', async (_req, res) => {
    res.json({
        showForm: {
            name: 'deleteGoalForm',
            form: {
...
router.post('/internal/form/delete-goal', async (req, res) => {
    const { confirm } = req.body;
    if (!confirm) {
        res.json({
            showToast: 'You did not confirm the deletion. If that was a mistake, please try again and enable the confirmation toggle before hitting delete.',
        });
        return;
    }
...
router.post('/internal/menu/erase-data', async (_req, res) => {
    res.json({
        showForm: {
            name: 'eraseDataForm',
            form: {
...
router.post('/internal/form/erase-data', async (req, res) => {
    const { username, userId, confirm } = req.body;
    if (!confirm) {
        res.json({
            showToast: 'You did not confirm the erasure. Please enable the confirmation toggle before proceeding.',
        });
        return;
    }
    if (!username && !userId) {
        res.json({
            showToast: 'User details were not provided. Please enter a username, user ID, or both.',
        });
        return;
    }
    ...
    try {
        if (resolvedUserId) {
            const typedUserId = resolvedUserId;
        ...
        res.json({
            showToast: 'Could not fetch all user details. Deletion will proceed, but may not catch all data. Please try again with the user ID if possible.',
        });
    }
    if (resolvedUserId) {
